...
def _check_password() -> bool:
    ...
    with col_c:
        ...
        st.markdown('<div class="gate-footer">Restricted access &nbsp;·&nbsp; <strong>EmpiricX</strong> v2.0</div>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    # ── Pricing Cards ──────────────────────────────────────────────────
    st.markdown("""
    <style>
    .pricing-section{margin:3rem auto 2rem;max-width:860px;text-align:center;}
    .pricing-heading{font-family:var(--serif);font-size:clamp(1.5rem,4vw,2rem);color:var(--t1);margin-bottom:.4rem;font-weight:700;}
    .pricing-heading span{color:var(--gold);font-style:italic;}
    .pricing-subline{font-size:.9rem;color:var(--t3);margin-bottom:2rem;}
    .pricing-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1.25rem;margin:0 auto;}
    .price-card{background:var(--panel);border:1px solid var(--b1);border-radius:var(--r-xl);padding:2rem 1.5rem 1.6rem;position:relative;overflow:hidden;transition:border-color .2s,box-shadow .2s;text-align:left;}
    .price-card::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,var(--gold),transparent);opacity:.5;transition:opacity .2s;}
    .price-card:hover{border-color:var(--b3);box-shadow:0 8px 40px rgba(0,0,0,.35);}
    .price-card:hover::before{opacity:1;}
    .price-card.featured{border-color:var(--gold-dk);background:linear-gradient(145deg,var(--panel),var(--panel-2));}
    .price-card.featured::before{opacity:1;background:linear-gradient(90deg,var(--gold),var(--gold-lt),transparent);}
    .price-badge{display:inline-block;font-family:var(--mono);font-size:.56rem;letter-spacing:2px;text-transform:uppercase;background:var(--gold-soft);border:1px solid var(--gold-mid);color:var(--gold);padding:3px 10px;border-radius:20px;margin-bottom:.85rem;}
    .price-name{font-family:var(--serif);font-size:1.1rem;font-weight:700;color:var(--t1);margin-bottom:.25rem;}
    .price-amount{font-family:var(--serif);font-size:2rem;font-weight:700;color:var(--gold);line-height:1;margin-bottom:.2rem;}
    .price-amount span{font-size:.85rem;font-weight:400;color:var(--t3);font-family:var(--sans);}
    .price-desc{font-size:.8rem;color:var(--t3);line-height:1.6;margin-bottom:1.25rem;min-height:48px;}
    .price-feats{list-style:none;padding:0;margin:0 0 1.5rem;display:flex;flex-direction:column;gap:7px;}
    .price-feats li{display:flex;align-items:flex-start;gap:8px;font-size:.82rem;color:var(--t2);line-height:1.4;}
    .price-feats li::before{content:'\\2726';font-size:.55rem;color:var(--gold);flex-shrink:0;margin-top:4px;}
    .price-cta{display:block;width:100%;padding:.7rem 1rem;border-radius:var(--r);font-family:var(--sans);font-size:.9rem;font-weight:600;text-align:center;text-decoration:none;transition:all .2s;cursor:pointer;box-sizing:border-box;}
    .price-cta-primary{background:linear-gradient(135deg,#d4a843 0%,#a87e2a 100%);color:#0a0700;border:none;}
    .price-cta-primary:hover{background:linear-gradient(135deg,#e8bc55 0%,#bc9035 100%);box-shadow:0 6px 22px rgba(212,168,67,.35);transform:translateY(-1px);}
    .price-cta-ghost{background:transparent;color:var(--t2);border:1px solid var(--b2);}
    .price-cta-ghost:hover{border-color:var(--gold);color:var(--gold);background:var(--gold-soft);}
    .app-footer{margin:3rem auto 1.5rem;max-width:860px;border-top:1px solid var(--b1);padding-top:2rem;text-align:center;}
    .footer-links{display:flex;flex-wrap:wrap;justify-content:center;gap:.75rem 1.75rem;margin-bottom:1.25rem;}
    .footer-links a{font-size:.82rem;color:var(--t3);text-decoration:none;transition:color .15s;font-family:var(--sans);}
    .footer-links a:hover{color:var(--gold);}
    .footer-copy{font-family:var(--mono);font-size:.62rem;letter-spacing:1.5px;text-transform:uppercase;color:var(--t4);}
    .footer-copy strong{color:var(--gold-dk);font-weight:500;}
    @media(max-width:640px){.pricing-grid{grid-template-columns:1fr;}}
    </style>

    <div class="pricing-section">
      <div class="pricing-heading">Simple, <span>Transparent</span> Pricing</div>
      <div class="pricing-subline">Choose the plan that fits your research workflow. No hidden fees.</div>
      <div class="pricing-grid">
        <div class="price-card">
          <div class="price-badge">Starter</div>
          <div class="price-name">Solo Researcher</div>
          <div class="price-amount">&#8358;4,500 <span>/ month</span></div>
          <div class="price-desc">Perfect for individual researchers and students getting started.</div>
          <ul class="price-feats">
            <li>Up to 10 papers / month</li>
            <li>Full empirical extraction</li>
            <li>CSV &amp; Excel export</li>
            <li>Email support</li>
          </ul>
          <a href="YOUR_FLUTTERWAVE_LINK_STARTER" class="price-cta price-cta-ghost" target="_blank" rel="noopener">Get Started &#8594;</a>
        </div>
        <div class="price-card featured">
          <div class="price-badge">Most Popular</div>
          <div class="price-name">Professional</div>
          <div class="price-amount">&#8358;12,000 <span>/ month</span></div>
          <div class="price-desc">Ideal for academics, consultants, and active research teams.</div>
          <ul class="price-feats">
            <li>Up to 50 papers / month</li>
            <li>Full extraction + synthesis</li>
            <li>CSV, Excel &amp; Word export</li>
            <li>Cross-paper AI synthesis</li>
            <li>Priority support</li>
          </ul>
          <a href="YOUR_FLUTTERWAVE_LINK_PRO" class="price-cta price-cta-primary" target="_blank" rel="noopener">Subscribe Now &#8594;</a>
        </div>
        <div class="price-card">
          <div class="price-badge">Institutional</div>
          <div class="price-name">Team &amp; Institution</div>
          <div class="price-amount">&#8358;35,000 <span>/ month</span></div>
          <div class="price-desc">For departments, labs, and multi-user research environments.</div>
          <ul class="price-feats">
            <li>Unlimited papers</li>
            <li>Full feature access</li>
            <li>Up to 5 team seats</li>
            <li>Bulk export &amp; batch mode</li>
            <li>Dedicated support</li>
          </ul>
          <a href="YOUR_FLUTTERWAVE_LINK_INSTITUTION" class="price-cta price-cta-ghost" target="_blank" rel="noopener">Contact Us &#8594;</a>
        </div>
      </div>
    </div>
    ...
    <footer class="app-footer">
      <div class="footer-links">
        <a href="YOUR_USER_GUIDE_LINK" target="_blank" rel="noopener">&#128214; User Guide</a>
        <a href="YOUR_SUPPORT_LINK" target="_blank" rel="noopener">&#128172; Contact Support</a>
        <a href="#" onclick="return false;">Privacy Policy</a>
        <a href="#" onclick="return false;">Terms of Use</a>
        <a href="#" onclick="return false;">FAQs</a>
      </div>
      <div class="footer-copy">&copy; 2025 <strong>EmpiricX</strong> &nbsp;&middot;&nbsp; Research Intelligence Engine &nbsp;&middot;&nbsp; v2.0 &nbsp;&middot;&nbsp; All rights reserved</div>
    </footer>
    """, unsafe_allow_html=True)
...
def page_synthesis():
    ...
    with c1: st.markdown(f'<p style="color:var(--t2);font-size:.88rem;margin:0"><strong style="color:var(--gold)">{len(papers)}</strong> papers ready for deep synthesis.</p>', unsafe_allow_html=True)
    with c2: run = st.button("&#128279; Run" if not synthesis else "&#128260; Re-run", type="primary", use_container_width=True)
    if run:
